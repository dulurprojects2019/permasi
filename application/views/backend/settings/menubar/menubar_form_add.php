            ...
            <form role="form" action="<?= site_url('settings/'.$controllers.'/add') ?>" method="post" enctype="multipart/form-data">
              <div class="card-body">
                <div class="form-group">
                  <input type="Text" name="mnb_name" class="form-control <?= form_error('mnb_name') == TRUE ? 'is-invalid' : ''; ?>" placeholder="Enter Menu Bar Name *">
                  <a style="color: red;"><?= form_error('mnb_name') ?></a>
                </div>
                <div class="form-group">
                  <label>Gambar Harus berukuran widht:74 pixels, height:29 pixels</label>
                  <div class="input-group">
                    <div class="custom-file">
                      <input type="file" name="mnb_image" class="custom-file-input <?= form_error('mnb_image') == TRUE ? 'is-invalid' : ''; ?>">
                      <label class="custom-file-label" for="exampleInputFile">Choose Menubar</label>
                    </div>
