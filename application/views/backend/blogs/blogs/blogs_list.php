          ...
          <div class="card-body table-responsive">
            <table id="example" class="table table-bordered table-striped">
              ...
              <tr>
                ...
                <td>
                  <?php $category =  explode(',', $data['blg_cat_id'])?>
                  <?php foreach($category as $cat) : ?>
                    <?php foreach($categories as $categ) : ?>
                    <?php if($cat == $categ['cat_id'] ) : ?>
                      <a style="color: <?= $categ['cat_color'] ?>"><?= $categ['cat_name'] ?></a>
                    <?php endif; ?>
                    <?php endforeach; ?>
                  <?php endforeach; ?>
                </td>
                <td><a style="color: red"><?= $data['blg_view_id'] ?></a></td>
                <td>
                  <?php if($data['blg_status'] == '1')
                  { ?>
                    <div class="btn-group btn-group-sm"><a href="<?= site_url('/blogs/'.$controllers.'/edit/status/'.$data['blg_slug']) ?>" class='btn btn-primary'>Publish</a></div>
                  <?php } else
                  { ?>
                    <div class="btn-group btn-group-sm"><a href="<?= site_url('/blogs/'.$controllers.'/edit/status/'.$data['blg_slug']) ?>" class='btn btn-warning'>Draft</a></div>
                  <?php } ?>
                </td>
                <td>
                  <div class="btn-group btn-group-sm">
                    <a href="#" class="btn btn-info" data-toggle="modal" data-target="#exampleModal<?= $data['blg_slug'] ?>"><i class="fas fa-eye"></i></a>
                    <a href="<?= site_url('/blogs/'.$controllers.'/edit/'.$data['blg_slug']) ?>" class="btn btn-success"><i class="fas fa-edit"></i></a>

                    <!-- only admin -->
                    <?php if($this->fungsi->user_login()->usr_lvl_id == '1') : ?>
                    <a href="<?= site_url('/blogs/'.$controllers.'/delete/'.encrypt_url($data['blg_id'])) ?>" onclick="return confirm('Are you sure you want to Remove?');" class="btn btn-danger"><i class="fas fa-trash"></i></a>
                    <?php endif ?>
                  </div>
                </td>
              </tr>
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <ul class="list-group list-group-unbordered mb-3">
                        ...
                        <?php if ( $data['blg_image'] == 'noimageblogs.png' ) : ?>
                          <div class="text-center">
                            <img class="img-fluid img-responsive"
                                 src="<?=base_url()?>assets/img/'.$controllers.'/noimages/noimageblogs.png"
                        ...
                        <li class="list-group-item">
                          Category View : <a style="color: red"><?= $data['blg_view_id'] ?></a>
                        </li>
                        <li class="list-group-item">
