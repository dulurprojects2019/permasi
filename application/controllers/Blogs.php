...
class Blogs extends CI_Controller {
	...
	{
		$data['blogs'] 			= $this->blogs_m->get_blogs();
	...
	public function add(){
		...
		$this->form_validation->set_rules('blg_cat_id[]', 'Blog Category', 'required');
		$this->form_validation->set_rules('blg_view_id', 'Blog Category View', 'required');
		...
		if($this->form_validation->run() === FALSE){
			...
			$this->blogs_m->add_blogs($image);
			redirect('blogs/blogs-list');
		}
	}

	public function edit($slug = NULL){
		$data['blogs'] 			= $this->blogs_m->get_blogsview($slug);
		$data['categories'] 	= $this->global_m->get_blogs_category();
		$data['iconbar'] 		= $this->global_m->get_iconbar();
		...
		if($this->form_validation->run() === FALSE){
			...
			if(!$this->upload->do_upload('blg_image')){
				...
				$data = $this->blogs_m->ambil_id_gambar($id);
			...
			$this->blogs_m->update_blogs($image);
			redirect('blogs/blogs-list');
		}
	}
