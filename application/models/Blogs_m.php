...
class Blogs_m extends CI_Model {
	...
	public function add_blogs($image){
		...
		$blog_category 			= implode(',', $this->input->post('blg_cat_id'));
		$data = array(
			'blg_title' 		=> $this->input->post('blg_title'),
			'blg_slug' 			=> strtolower($slug),
			'blg_cat_id' 		=> $blog_category,
			'blg_view_id' 		=> $this->input->post('blg_view_id'),
			'blg_image' 		=> $image,
	...
	public function update_blogs($image){
		...
		$data = array(
			...
			'blg_tags' 			=> $tags,
			'blg_status' 		=> $this->input->post('blg_status'),
			'blg_edited_at' 	=> $this->input->post('blg_created_at', array('type' => 'timestamp')),
